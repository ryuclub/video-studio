...
function homeLiving(s = 9) {
  ...
  // 挂历：1.35 人高
  //
  // ⚠ **2026-08-25 整体左移 42px（x 120 → 78）。**
  // 原来右边缘在 x=350，而收尾卡的钩子字（`hookStrip`）画死在 y=250、居中，
  // 「老马的第 1874 天」实测横跨 **x 324–756**、墨迹 y 268–313 —— 挂历右侧那 26px
  // 正好压在钩子字的头一个字上，两处字叠成一团。尾卡事实句（y≈410–450）同理，
  // 也蹭到挂历的右下角。
  //
  // **这就是 BASE §2「换新场景之前，先量一遍收尾卡那一片地」说的那块地**，
  // 而它只在最后两秒露出来 —— 逐镜看静帧看不见，非得渲完整条或者单抽定格帧才发现。
  // 让的是**宽度方向**不是高度：挂历是挂在墙上的，往下挪就撞脸，往右挪就占掉字幕那半幅。
  //
  // ⚠ 挂历上那块便签（`plaque.mjs` 的 home-living `sticky` 位）要跟着一起挪，
  // 不然字会飘到挂历外面去。
  // ⚠ **高度也压过一次：330 → 140**（同一天，同一个原因）。
  // 光左移不够 —— 日子牌那行字**比 BASE §2 记的宽得多**：那条注释写的是
  // 「字号 52、横跨 x 324–756」，实测现在是 **x≈250–840**（字号 72）。
  // 横向躲不开，就按 BASE 的正解躲纵向：**让的是高度**。
  // 压到 140 之后挂历底边落在 y≈229，日子牌墨迹从 y≈270 起 —— 让开 41px。
  const CAL_H = 140;
  o += shape(rectPts(78, up(1.26), 230, CAL_H), { fill: LIGHT, hatchGap: 0, seed: s + 1, w: 3.6 });
  o += stroke([[90, up(1.26) + 48], [298, up(1.26) + 44]], { color: INK, w: 2.8, passes: 1, amp: 2, seed: s + 2, op: 0.6 });
  for (let r = 0; r < 4; r++)
    o += stroke([[98, up(1.26) + 72 + r * 18], [290, up(1.26) + 70 + r * 18]],
