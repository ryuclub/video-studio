...
/**
 * 背景音乐响度。**已定档 −38**，比人声低 17 LU。
 *
 * 四轮的账：v1 出过 −34 / −38 / −42 三档，人选了 −42（低 21 LU）；
 * 《方丈记》两期出完后反馈「可以再稍微大一些」，抬到 −40；
 * 《枕草子》两期（听的就是 −40）出完后再次反馈「之前的太小了」，抬到 −38。
 *
 * **这一档回到了 v1 里被放过去的那个数**，是有意的，不是忘了上一轮的判断：
 * v1 那次是三档并排比、人挑了最轻的一档，回答的是「哪档最合适」；
 * 后两次是连着听完四期成片之后提的，回答的是「长期听下来够不够」。
 * 两个问题不一样，后者的证据更重。
 *
 * 步长跟上一次一致（+2 LU）—— +1 到 −39 大概率听不出来，
 * 而「听不出来」会让下一轮反馈没法解读。
 * 要是 −38 又过了头，下一格是 −39，别直接跳回 −40。
 */
export const BGM_LUFS = -38;
/** 音乐淡入淡出（秒） */
