...
/**
 * 老马（碎嘴老马 · 段子）。**它从 `OUT_JOKE` 里分出来了**（2026-08-23）。
 *
 * ── 为什么单独一棵树 ──
 *
 * 别的线是「一条片子一个目录，日期是渲染日」。老马这条要的是**排期账本**：
 * 目录名带发布日和时刻、发完整个目录 mv 进 `_已发/`、`_待发/` 里几个目录就是
 * 还剩几期（`horse/SCHEDULE.md`）。这两种用法塞在一个平目录里做不到 ——
 * 那边的目录名要稳（一堆代码按 `*_<id>` 反查），这边的目录名**要能随时改**
 * （改期只动前缀）。
 *
 * 结构：`老马/段子/{_待发,_已发}/<日期>_<时刻>JST_段子-<天数号>/`
 */
export const OUT_LAOMA = `${OUT_ROOT}/老马`;
/** 说书（聊斋） */
