...
>
> **怎么出片：三条线拆开写，共用的部分在 BASE。**（2026-08-25 起，
> `../老马出片方案.md` 是拆分之前的那一份，只留作历史）
>
> | | 文档 |
> |---|---|
> | 共用层（rig / 场景 / 音色 / 管线 / 动效） | [BASE.md](BASE.md) |
> | 单点式（铺垫—转折—落点，带日子牌） | [单点式_出片方案.md](单点式_出片方案.md) |
> | 累积式（排比自嘲，不带日子牌） | [累积式_出片方案.md](累积式_出片方案.md) ＋ [累积式_字幕.md](累积式_字幕.md) ＋ [累积式_出片计划.md](累积式_出片计划.md)（35 条稿件库） |
> | 长片（5 分钟横屏，说破一次） | [长片_出片方案.md](长片_出片方案.md) ＋ [长片_稿件规范.md](长片_稿件规范.md)（**它自己的渲染器在 `长片/`**） |
>
> ⚠ **三份分叉文档互不参照，也不互相继承缺省值** ——
> 归属判据一句话：**这条规则换了体裁就该反过来，它属于分叉文档；换了体裁还一样，它属于 BASE。**
>
...
| `bull-side/` | **老牛**：`bull_stand.svg` 结构跟 `horse_only.svg` 对等（五个口型、pupil、head 都齐），交接说明在 `BULL_HANDOFF.md`。长片那套渲染器直接用它 |
| `长片/longform.cjs` | **长片专用渲染器**（1280×720 横屏，六个场景 / 两个 rig / 鱼缸 / 字幕 / 场景标牌）。跟上面这套竖屏的东西**不共用一行代码** —— 出帧走 `tools/long-frames.mts` |
| `render.mjs` | 命令行入口 |
