...
const SHOT_TYPES = [
  ...
  },
  {
    ...
    how: '`intro`（秒）',
    when: '每条片子开头，交代环境',
    note: '镜头从 1.06 缓推到 1.0；角色从画外滑入。**先出人、滑到位了才开口**',
  },
  {
    name: '开场大字 · 场景（出场档 ②）',
    how: '`opening: { style: "voice-first" }`',
    when: '老马线的第二档出场风格。首帧要对比度、要一个数把人钩住的时候用',
    note:
      '**场景照常出，人不出，第 1 句里那个数压成大字盖在场景上**；**音轨从 0.0 起**' +
      '（`intro` 强制归零）。第 1 句一停字就撤，角色在那半秒静音里滑进来。' +
      '入场动画本身没改，只改了它跟音轨的对齐关系。' +
      '**角色的出场变成一个兑现** —— 观众先被那句话钩住，才看到说话的人是谁。' +
      '⚠ 做过一版整幅黑底的，黑得太久（第 1 句要念三四秒），而且场景这条信息白白晚到四秒',
  },
  {
    name: '开场物件特写（出场档 ③）',
    how: '`opening: { style: "object-first", frameSubject, frameText }`',
    when: '物件是画得出特写的实物，而且要抢信息流里那 120px 缩略图',
    note:
      '**只有物件，没有场景没有人**（首帧规范 §五 把「整幅场景图」列成禁令：信息量太散，' +
      '观众要花半秒扫画面，同时声音在讲第一句，注意力分裂）。字在物件正下方 y=0.72H，' +
      '**其中的数字标朱红** —— 首帧唯一的一点红，跟收尾卡「第 N 天」一红一黑开合对上。' +
      '⚠ 验收看 `npm run frame` 出的 **120px 缩略图**：物件认得出吗？数字看得见吗？' +
      '⚠ 抽象物件（系统、群）画不了特写，那种稿子走 ②',
  },
];
